...
if (is.null(params$punctuation)) {
  ...
  code.rate <- paste0("$$\\frac{",length(params$punctuation),"}{",sum(params$punctuation) * factor,"}$$")
}
```

- Kode-Rate : `r code.rate`
`r if(params$turbo) paste0("- Interleaver : (", paste(params$permutation, collapse = ", "), ")") `
...
df <- params$dataframe
ggplot2::ggplot(df,ggplot2::aes(db))+ggplot2::geom_line(ggplot2::aes(y = ber))+ggplot2::scale_y_log10()
