---
title: "Convolution Decode Punktiert"
...
code.rounded <- round(code, digits = 1)
code.rounded <- paste0(code.rounded, collapse=",")
code.hard <- ifelse(code >= 0, 0, 1)
code.hard2 <- paste0(code.hard, collapse = ", ")
code.hard <- paste0(code.hard, collapse=",")
punctured.code.hard <- ifelse(params$punctured.code >= 0, 0, 1)

table.def <- "\\begin{tabular}{r l}"
row1 <- paste0("input: & (",code.rounded,")\\\\")
row2 <- paste0("hard input: & (",code.hard,")\\\\")
table.end <- "\\end{tabular}"

decode.input <- paste0(table.def, row1, row2, table.end)
...
if (ncol(trellis) > 10) {
  xfactor = max(0.75, exp((10-ncol(trellis))/15))
} else {
  xfactor = 1
}
...
- Punktierter Code: (`r punctured.code.hard`)
- Punktierungs-Matrix : $$\begin{pmatrix}  `r punctuation.print`  \end{pmatrix}$$
- Depunktierter Code: (`r code.hard`)

## Trellis Diagram

input: (`r code.rounded`) = (`r code.hard`)
\vfill
