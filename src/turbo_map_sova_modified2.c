	...
	for(int k = 1; k < N+1; k++)
	{
		for(int s = 0; s < M; s++)
		{
			for(int i = 0; i < 2; i++)
			{
				...
				if(previous[s][i] != -1)
				{
					Max[i] = Metric[k-1][previous[s][i]]  +  (i * 2-1) * (Lc * x_d[k-1] + La[k-1]) + ((((parity[previous[s][i]][i] >> use_parity_index) & 1) > 0) * 2 -1) * Lc * p_d[k-1];	
				} else
				{
					Max[i] = -1000;
				}
...
)
{
	...
	int previous[amount_states][2];   //previous[m][i] = previous state (this comes to state m with databit = i)
	...
	for(int i = 0; i < amount_states; i++)
	{
		for(int j = 0; j < 2; j++)
		{
			previous[i][j] = previous_in[i*2+j]; 
